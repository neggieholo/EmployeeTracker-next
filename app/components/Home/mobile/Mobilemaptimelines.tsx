...
import MobileUserTimelineMap from './MobileUserTimelineMap';
import MobileTimelineMapControls from './MobileTimelineControls';
import { fetchTodaysTimelines, fetchFilteredMaps, GroupedTimelines } from '@/app/Services/apis';
import { toast } from 'react-toastify';
import { Map as MapIcon, Search} from 'lucide-react';

const MobileMapTimelines = () => {
  ...
  return (
    <div className="flex flex-col gap-6 p-2 min-h-full bg-base-100">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-black uppercase italic tracking-tighter flex items-center gap-2">
            <MapIcon className="text-primary" size={32} />
            Route <span className="text-primary">Timelines</span>
          </h1>
          <div className="flex items-center gap-2 mt-1">
            <div className="badge badge-outline text-gray-500 badge-sm font-bold opacity-70">
              {startDate ? `${startDate} to ${endDate || startDate}` : 'Today'}
            </div>
          </div>
        </div>

        <MobileTimelineMapControls
              ...
              <div key={uid} className="card bg-base-200 shadow-sm border border-base-300">
                <div className="card-body p-4">
                  ...
                  <div className="h-100 rounded-2xl overflow-y-auto border-2 border-base-300">
                    <MobileUserTimelineMap userId={uid} timeline={filteredTimelines[uid]} />
                  </div>
                </div>
              </div>
            ))}
...
export default MobileMapTimelines;
